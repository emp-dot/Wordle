...
class WordGenerator {
    ...
    static func generateRandomWord() -> String? {
        ...
        for word in possibleWords {
            // Randomly select a letter from the current word
            if let randomLetter = word.randomElement() {
                // Append the selected letter to the combined word
                combinedWord.append(randomLetter)
            }
        }
        ...
        return combinedWord.isEmpty ? nil : combinedWord
        // END YOUR CODE HERE
    }
}
